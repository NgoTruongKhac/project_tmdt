...
export default function Navbar() {
  ...
  return (
    <div className="navbar bg-white shadow-soft px-4 sm:px-6 py-3 sticky top-0 z-50">
      ...
      <div className="navbar-center hidden lg:flex w-full max-w-md"></div>

      {/* 3. Phần Icons và Nút xác thực (Bên phải) */}
      <div className="navbar-end flex items-center gap-1 sm:gap-2">
        {/* Icon Bell hiển thị khi đã đăng nhập */}
        {user && (
          <>
            <button
        ...
        {user ? (
          <div className="ml-2 dropdown dropdown-end">
            ...
            <div
              tabIndex={0}
              className="dropdown-content mt-3 w-52 z-[100] bg-white rounded-xl shadow-lg border border-neutral-100 overflow-hidden"
            >
              {/* Header */}
              ...
              {/* Menu */}
              <ul className="menu p-2 w-full gap-0.5">
                {/* Chỉ hiển thị Trang quản lý nếu role là designer */}
                {user.role === "DESIGNER" && (
                  <li>
                    <Link
                      to="/designer-manage/dashboard"
                      className="flex items-center gap-2 text-neutral-700 hover:text-primary hover:bg-primary-50 font-medium rounded-lg px-3 py-2"
                    >
                ...
                <li>
                  <button
                    ...
                    className="flex items-center gap-2 w-full text-left text-red-500 hover:text-red-600 hover:bg-red-50 font-medium rounded-lg px-3 py-2"
                  >
