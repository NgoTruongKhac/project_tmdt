...
const getServiceTitle = (service: IServicePackage) =>
    service.name || service.title || "Gói dịch vụ thiết kế";
...
const PackageDetail: React.FC = () => {
    ...
    // Hàm xử lý thanh toán thực sự (gọi từ trong Modal)
            ...
            {isModalOpen && (
                <div style={styles.modalOverlay}>
                    <div style={styles.modalContent}>
                        <h2 style={styles.modalTitle}>Chi tiết đặt hàng</h2>
                        <p style={styles.modalSub}>Vui lòng cung cấp thông tin để designer thực hiện</p>

                        <div style={styles.formGroup}>
                            <label style={styles.label}>Tải lên ảnh gốc cần chỉnh sửa:</label>
                            <input
                                type="file"
                                accept="image/*"
                        ...
                        <div style={styles.formGroup}>
                            <label style={styles.label}>Yêu cầu chi tiết:</label>
                            <textarea
                                rows={4}
                                value={userNote}
                                onChange={(e) => setUserNote(e.target.value)}
                                placeholder="Ví dụ: Chỉnh màu sắc tươi sáng hơn, xóa vật thể..."
                    ...
                    <div style={styles.infoSection}>
                        {designer && (
                            <div style={styles.designerRow}>
                                <img
                                    src={designer.profilePicture || `https://ui-avatars.com/api/?name=${encodeURIComponent(designer.fullName)}`}
                        ...
                        <p style={styles.descriptionText}>{service.description || "Chưa có mô tả."}</p>
                    </div>
                </div>
                ...
                <div style={styles.rightCol}>
                    <div style={styles.pricingCard}>
                        ...
                        <div style={styles.rewardBox}>
                            ...
                            <div style={styles.rewardControl}>
                                <input
                                    ...
                                    onChange={(e) => setRewardPointsToUse(Math.max(0, Math.min(maxRewardPointsToUse, Number(e.target.value))))}
                        ...
                        <div style={styles.serviceCommitment}>
                            <div style={styles.featureItem}>✓ Thời gian: {service.deliveryTime} ngày</div>
                            <div style={styles.featureItem}>✓ Số lần sửa: {service.revisions} lần</div>
                            <div style={styles.featureItem}>✓ Định dạng PNG, JPG, SVG</div>
                            <div style={styles.featureItem}>✓ Toàn quyền thương mại</div>
                        </div>

                        <button style={{...styles.orderBtn, backgroundColor: "#8b5cf6"}} onClick={handleOrderClick}>
                            Đặt hàng
                        </button>
                    ...
                    {relatedServices.map((item) => (
                        <div key={item._id} style={styles.relCard} onClick={() => navigate(`/package/${item._id}`)}>
                            <div style={styles.relImgBox}>
                                <img src={getProtectedServiceImage(item.thumbnail || item.images?.[0])} style={styles.relImg} alt="rel" />
                            </div>
...
const styles: { [key: string]: React.CSSProperties } = {
    ...
    modalSub: { fontSize: "14px", color: "#666", marginBottom: "20px" },
    formGroup: { marginBottom: "15px", textAlign: "left" },
    label: { display: "block", fontSize: "14px", fontWeight: "700", marginBottom: "5px" },
    fileInput: { width: "100%", fontSize: "13px" },
    textarea: { width: "100%", padding: "10px", borderRadius: "8px", border: "1px solid #ddd", outline: "none", resize: "none", fontSize: "14px" },
    modalActions: { display: "flex", justifyContent: "flex-end", gap: "10px", marginTop: "20px" },
    cancelBtn: { padding: "10px 20px", borderRadius: "8px", border: "1px solid #ddd", background: "white", cursor: "pointer" },
    payNowBtn: { padding: "10px 20px", borderRadius: "8px", border: "none", background: "#0075f2", color: "white", fontWeight: "700", cursor: "pointer" },
