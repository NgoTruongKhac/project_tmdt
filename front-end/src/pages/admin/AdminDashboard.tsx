...
function MetricCard({ title, value, icon: Icon, accentClassName, valueClassName, subtitle }: MetricCardProps) {
    return (
...
export default function AdminDashboard() {
    ...
    const hasTopServiceData = topServices.length > 0;
        ...
        <div className="space-y-6">
            ...
            {isLoading ? (
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    ...
                    <LoadingCard className="h-[22rem] md:col-span-2" />
                    <LoadingCard className="h-[22rem] md:col-span-2" />
                    <LoadingCard className="h-[24rem] xl:col-span-4" />
                </div>
            ) : dashboardData ? (
                <>
                    <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        ...
                        <MetricCard
                            ...
                            subtitle="Toàn bộ dịch vụ đã tạo"
                        />
                        <MetricCard
                            ...
                            subtitle="Cần quản trị viên xử lý ngay"
                        />
                        <MetricCard
                            title="Tổng lượt bán"
                            ...
                            subtitle="Tổng đơn hàng thành công"
                        />
                    </div>
                    ...
                    <div className="grid gap-4 xl:grid-cols-2">
                        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                            <div className="mb-4 flex items-center justify-between gap-3">
                                <div>
                                    <h2 className="text-lg font-semibold text-gray-900">Tỷ lệ vai trò người dùng</h2>
                                    <p className="text-sm text-gray-500">Customer, Designer, Admin</p>
                                </div>
                            </div>
                                        ...
                                        <PieChart>
                                            ...
                                            >
                                                {roleData.map((entry, index) => (
                                                    <Cell key={entry.name} fill={COLORS[index % COLORS.length]} className="capitalize" />
                                                ))}
                                            </Pie>
                                            <Tooltip formatter={(value: any) => [formatNumber(Number(value)), "Người dùng"]} />
                        ...
                        <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                            <div className="mb-4 flex items-center justify-between gap-3">
                                <div>
                                    <h2 className="text-lg font-semibold text-gray-900">Phân bố dịch vụ theo danh mục</h2>
                                    <p className="text-sm text-gray-500">Số lượng dịch vụ ở từng loại</p>
                                </div>
                            </div>
                    ...
                    <div className="rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
                        <div className="mb-4 flex items-center justify-between gap-3">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">Sắp xếp theo số lượng đã bán</p>
                            </div>
                            ...
                            {hasTopServiceData ? (
                                <ResponsiveContainer width="100%" height="100%">
                                    ...
                                    >
                                        ...
                                        <Bar dataKey="soldCount" radius={[0, 10, 10, 0]} fill="#10B981" />
                                    </BarChart>
                                </ResponsiveContainer>
                            ) : (
                                <div className="flex h-full items-center justify-center rounded-xl bg-gray-50 text-sm text-gray-500">
                                    Chưa có dữ liệu top 5 dịch vụ bán chạy.
                                </div>
                            )}
                        </div>
                    </div>
